...
# Place some input urls to site_data\\0_1_input_urls.txt file in format: whatever;;;link one link per line.

# Control which modules will run in Link_Module_Driver. Usual case leaving all modules in workinf condition
# WARNING! When running for the first time on same site, next module works after all modules before it done their job;
# Dupli check won't allow do anything with site if the site link is not removed from all site link management files.
# 
# Execute the script
...
class Site_Link_Manager(Common_Paths):
    def __init__(self, input_url):
        ...
        self.vg_file_name = "2_1_good_veri.txt"
        self.vb_file_name = "2_2_bad_veri.txt"
        ...
        self.vg_file_path = self.main_path + self.vg_file_name
        self.vb_file_path = self.main_path + self.vb_file_name
...
class HTML_File_Maker(Common_Paths):
    def __init__(self, input_url):
        ...
        self.failed_retrieving_html_counter = 0
...
class Translator(Common_Paths):
    def __init__(self, input_url):
        ...
        # making dinamic filename and path class level accesable
        self.eng_p_text_f_name = ""
        self.error_eng_p_text_f_name = ""
        self.eng_p_text_f_path = ""
        self.p_text_f_name = ""
        self.page_text = ""

        self.max_page_length = 2000    # in case where we have too much text to be translated
                                        # page text is devided into part files

        self.new_page_list = []

        self.loaded_driver = self.load_chrome_webdriver()

        

    def load_chrome_webdriver(self):
        """ LOADS CHROME DRIVER WITH EXTENSION 
            SEPARATE METHOD FOR CHROME DRIVER RELOADING
            IN CASE OF TRANSLATING FAILURE"""

        # starts chrome with translate app
        options = Options()
        options.add_argument('load-extension=C:\\Users\\MuMu\\AppData\\Local\\Google\\Chrome\\User Data\\Default\\Extensions\\aapbdbdomjkkjkaonfhkkikfgjllcleb\\2.0.6_0')
    ...
    def get_text(self):
        ...
        while len(text_output) == 0:
            ...
            print "get_text: BIG TRANSLATION CYCLE NO.", track_while_loops

            if track_while_loops == 3:
                print "get_text: RELOADING CHROME DRIVER AFTER THREE TRANSLATION ATEMPTS"
                self.loaded_driver.quit()
                self.loaded_driver = self.load_chrome_webdriver()
            # eternal loop safeguard
            elif track_while_loops > max_while_loops:
                # raise error
                print "get_text: MAXIMUM TRIES TO TO TRANSLATE THE SAMPLE EXEEDED \npath: %s\\%s\\eng_TXT\\%s" % (main_path, self.domain, error_eng_p_text_f_name)
                ...
                print "get_text: ATTEMPTING TO TRANSLATE PAGE OR PART"

            # getting translate popup as html page:
            self.loaded_driver.get("chrome-extension://aapbdbdomjkkjkaonfhkkikfgjllcleb/popup.html")
            print "get_text: SLEEPING %d seconds, after EXTENSION GET" % ext_get_sleep
            time.sleep(ext_get_sleep)

            print "get_text: POPULATING CLIPBOARD with text from file...."
            pyperclip.copy(self.page_text)
            print "get_text: Sleeping after copying to clipboard for %d s" % text_copy_to_clip_sleep
            time.sleep(text_copy_to_clip_sleep)

            # Finding text input element
            text_input_el = self.loaded_driver.find_element_by_id('text-input')
            #sending command via selenium Keys
            text_input_el.send_keys(Keys.CONTROL, 'v')
            print "get_text: Sleeping after pasting for %d" % text_paste_sleep
            time.sleep(text_paste_sleep)
            #submit to translate
            print "get_text: Pressing return"
            text_input_el.send_keys(Keys.RETURN)
            
            text_output_tries = 0 # initial number of small while loop tries
            max_text_output_tries = 5

            # skips find output element and take text when trying for the first time
            if track_while_loops == 0 and self.eng_p_text_f_name == "eng_page_0_text.txt":
                track_while_loops += 1
                continue

            # small while cycle waiting for input to be processed
            while text_output_tries < max_text_output_tries and len(text_output) < 1:
                print "get_text: SMALL TRANSLATE CYCLE: GETTING RESULTS TRY NO. %d OUT OF %d" % (text_output_tries, max_text_output_tries)

                print "get_text: sleeping after submition for %d s. Waiting for text to be translated" % text_translate_sleep
                time.sleep(text_translate_sleep)

            #find output element, take text
                try:
                    text_output_el = self.loaded_driver.find_element_by_xpath('//*[@id="translation"]/div[6]')

                    text_output = text_output_el.text
                    print "get_text: TRANS_TEST_1: input sample\n%r" % self.page_text
                    print "get_text: TRANS_TEST_1: text_sample_ouput\n%r" % text_output
                    
                    return text_output

                except:
                    print "get_text: THE TRANSLATED SAMPLE WAS NOT GENERATED"

                text_output_tries +=1

            track_while_loops += 1 # incrementing tries

    def get_text_parts(self, **rewrite):

        start = 0
        end = 0
        page_text_copy = copy(self.page_text)
        full_output = ""

        # tracking parts numbers
        estimated_parts = math.ceil(len(self.page_text) / self.max_page_length)
        part_no = 1
        #/

        while end < len(page_text_copy):
            print "get_text_parts: GETTING OUTPUT TEXT PART NO. %d OUT OF %d" % (part_no, estimated_parts)
         
            end = start + self.max_page_length
            page_text_sample = page_text_copy[start:end]
            start = start + self.max_page_length
            self.page_text = page_text_sample
            if rewrite["rewrite"]:
                print "get_text_parts: TRANSLATING IN REWRITE MODE"
                text_output = self.get_text()

            elif not os.path.exists(self.eng_p_text_f_path):
                print "get_text_parts: TRANSLATING IN ONLY ONCE MODE"
                text_output = self.get_text()

            else:
                print "get_text_parts: SKIPPING FILE, ALREADY TRANSLATED, BREAKING PARTS WHILE LOOP"
                text_output = ""
                break
            full_output = full_output + text_output
...
class Link_Module_Driver(object):
    ...
    def __init__(self):
        ...
        self.site_failed_part_path = "site_data\\0_3_sites_failed.txt"

        self.link_input_path =  self.pc_specific_part_path + self.static_part_path
        self.site_failed_path = self.pc_specific_part_path + self.site_failed_part_path
